   ...
   Pacer::setPrefix("Omega:");
   {
            ...
            for (int K = 0; K < Nz; ++K) {
               ...
               for (int M = 0; M < Nm; ++M) {
                  ...
                  for (int N = 0; N < Nn; ++N) {
                     ...
                     TestHost5DR4(I, J, K, M, N) =
                         (NIndx + EpsR4) * FacR4[MyTask];
                     TestHost5DR8(I, J, K, M, N) =
                         (NIndx + EpsR8) * FacR8[MyTask];
                  }
               }
            }
         }
      ...
      /*
         ...
         if (isum4 == iref2) then
            ...
            print *, 'Global sum prod irange array2d integer: FAIL', isum4,
         iref2 endif if (lsum4 == lref2) then print *, 'Global sum prod irange
         array2d integer(8): PASS' else print *, 'Global sum prod irange array2d
         integer(8): FAIL', lsum4, lref2 endif deallocate(darray2d, dmask2d,
         rarray2d, rmask2d, & iarray2d, imask2d, larray2d, lmask2d)
         ...
         if (isum4 == iref2) then
            ...
            print *, 'Global sum prod irange array3d integer: FAIL', isum4,
         iref2 endif if (lsum4 == lref2) then print *, 'Global sum prod irange
         array3d integer(8): PASS' else print *, 'Global sum prod irange array3d
         integer(8): FAIL', lsum4, lref2 endif deallocate(darray3d, dmask3d,
         rarray3d, rmask3d, & iarray3d, imask3d, larray3d, lmask3d)
         ...
         if (isum4 == iref2) then
            ...
            print *, 'Global sum prod irange array4d integer: FAIL', isum4,
         iref2 endif if (lsum4 == lref2) then print *, 'Global sum prod irange
         array4d integer(8): PASS' else print *, 'Global sum prod irange array4d
         integer(8): FAIL', lsum4, lref2 endif deallocate(darray4d, dmask4d,
         rarray4d, rmask4d, & iarray4d, imask4d, larray4d, lmask4d)
         ...
         if (isum4 == iref2) then
            ...
            print *, 'Global sum prod irange array5d integer: FAIL', isum4,
         iref2 endif if (lsum4 == lref2) then print *, 'Global sum prod irange
         array5d integer(8): PASS' else print *, 'Global sum prod irange array5d
         integer(8): FAIL', lsum4, lref2 endif deallocate(darray5d, dmask5d,
         rarray5d, rmask5d, & iarray5d, imask5d, larray5d, lmask5d)
         ...
         if (isum4 == iref2) then
            ...
            print *, 'Global sum prod irange array6d integer: FAIL', isum4,
         iref2 endif if (lsum4 == lref2) then print *, 'Global sum prod irange
         array6d integer(8): PASS' else print *, 'Global sum prod irange array6d
         integer(8): FAIL', lsum4, lref2 endif deallocate(darray6d, dmask6d,
         rarray6d, rmask6d, & iarray6d, imask6d, larray6d, lmask6d)
         ...
         flush(6)
         call MPI_Barrier(MPI_COMM_WORLD, ierr)
         allocate(dsum1n(nFields), dsum2n(nFields), dsum3n(nFields),
         dsum4n(nFields), & rsum1n(nFields), rsum2n(nFields), rsum3n(nFields),
         rsum4n(nFields), & isum1n(nFields), isum2n(nFields), isum3n(nFields),
         isum4n(nFields), & lsum1n(nFields), lsum2n(nFields), lsum3n(nFields),
         lsum4n(nFields)) allocate(darray0n(nFields), rarray0n(nFields), &
         ...
         call MPI_Bcast(randoms, nRanks, MPI_REAL, masterTask, MPI_COMM_WORLD,
         ierr)
         ...
         if (dsum1n(1) == dsum1n(2) .and. dsum1n(1) == dsum1n(3) .and. &
             ...
             print *, 'Multi-field double scalar not sensitive to roundoff :
         WARN', dref, dsum1n print *, 'Multi-field double scalar data:', myRank,
         darray0n endif if (rsum1n(1) == rsum1n(2) .and. rsum1n(1) == rsum1n(3)
         .and. & rsum1n(2) == rsum1n(3)) then print *, 'Multi-field real scalar
         not sensitive to roundoff : WARN', rref, rsum1n print *, 'Multi-field
         real scalar data:', myRank, rarray0n endif
         ...
         if (dsum1n(1) == dref .and. dsum1n(2) == dref .and. &
            ...
            print *, 'Reproducibility and nfld scalar double: FAIL', dref,
         dsum1n endif if (rsum1n(1) == rref .and. rsum1n(2) == rref .and. &
             rsum1n(3) == rref) then
         ...
         if (isum1n(1) == iref .and. isum1n(2) == iref .and. &
            ...
            print *, 'Reproducibility and nfld scalar integer: FAIL', iref,
         isum1n endif if (lsum1n(1) == lref .and. lsum1n(2) == lref .and. &
             lsum1n(3) == lref) then
            print *, 'Reproducibility and nfld scalar integer(8): PASS'
         else
            print *, 'Reproducibility and nfld scalar integer(8): FAIL', lref,
         lsum1n endif deallocate(darray0n, rarray0n, iarray0n, larray0n)
         deallocate(randoms)
         ...
         dsum4n = mpas_globalSumNfld(darray1n, dmask1n, MPI_COMM_WORLD,
         indxRange) rsum4n = mpas_globalSumNfld(rarray1n, rmask1n,
         MPI_COMM_WORLD, indxRange) isum4n = mpas_globalSumNfld(iarray1n,
         imask1n, MPI_COMM_WORLD, indxRange) lsum4n =
         mpas_globalSumNfld(larray1n, lmask1n, MPI_COMM_WORLD, indxRange)

         if (dsum1n(1) == dref .and. dsum1n(2) == dref .and. &
             dsum1n(3) == dref) then
            print *, 'Multi-field and reproducibility 1d double: PASS'
         else
            print *, 'Multi-field and reproducibility 1d double: FAIL', dref,
         dsum1n endif if (rsum1n(1) == rref .and. rsum1n(2) == rref .and. &
             rsum1n(3) == rref) then
            print *, 'Multi-field and reproducibility 1d real: PASS'
         else
            print *, 'Multi-field and reproducibility 1d real: FAIL', rref,
         rsum1n endif if (isum1n(1) == iref .and. isum1n(2) == iref .and. &
             isum1n(3) == iref) then
            print *, 'Multi-field and reproducibility 1d int: PASS'
         else
            print *, 'Multi-field and reproducibility 1d int: FAIL', iref,
         isum1n endif if (lsum1n(1) == lref .and. lsum1n(2) == lref .and. &
             lsum1n(3) == lref) then
            print *, 'Multi-field and reproducibility 1d int(8): PASS'
         else
            print *, 'Multi-field and reproducibility 1d int(8): FAIL', lref,
         lsum1n endif isum1 = 0 isum2 = 0 isum3 = 0 isum4 = 0 do nfld=1,nFields
         ...
         if (isum1 > 0) then
            print *, 'Multifield range/mask 1d double: FAIL',
         dsum2n,dsum3n,dsum4n else print *, 'Multifield range/mask 1d double:
         PASS' endif if (isum2 > 0) then print *, 'Multifield range/mask 1d
         real: FAIL', rsum2n,rsum3n,rsum4n else print *, 'Multifield range/mask
         1d real: PASS' endif if (isum3 > 0) then print *, 'Multifield
         range/mask 1d int: FAIL', isum2n,isum3n,isum4n else print *,
         'Multifield range/mask 1d int: PASS' endif if (isum4 > 0) then print *,
         'Multifield range/mask 1d int(8): FAIL', lsum2n,lsum3n,lsum4n else
         ...
         dsum4n = mpas_globalSumNfld(darray2n, dmask2n, MPI_COMM_WORLD,
         indxRange) rsum4n = mpas_globalSumNfld(rarray2n, rmask2n,
         MPI_COMM_WORLD, indxRange) isum4n = mpas_globalSumNfld(iarray2n,
         imask2n, MPI_COMM_WORLD, indxRange) lsum4n =
         mpas_globalSumNfld(larray2n, lmask2n, MPI_COMM_WORLD, indxRange)

         if (dsum1n(1) == dref .and. dsum1n(2) == dref .and. &
             dsum1n(3) == dref) then
            print *, 'Multi-field and reproducibility 2d double: PASS'
         else
            print *, 'Multi-field and reproducibility 2d double: FAIL', dref,
         dsum1n endif if (rsum1n(1) == rref .and. rsum1n(2) == rref .and. &
             rsum1n(3) == rref) then
            print *, 'Multi-field and reproducibility 2d real: PASS'
         else
            print *, 'Multi-field and reproducibility 2d real: FAIL', rref,
         rsum1n endif if (isum1n(1) == iref .and. isum1n(2) == iref .and. &
             isum1n(3) == iref) then
            print *, 'Multi-field and reproducibility 2d int: PASS'
         else
            print *, 'Multi-field and reproducibility 2d int: FAIL', iref,
         isum1n endif if (lsum1n(1) == lref .and. lsum1n(2) == lref .and. &
             lsum1n(3) == lref) then
            print *, 'Multi-field and reproducibility 2d int(8): PASS'
         else
            print *, 'Multi-field and reproducibility 2d int(8): FAIL', lref,
         lsum1n endif isum1 = 0 isum2 = 0 isum3 = 0 isum4 = 0 do nfld=1,nFields
            if (dsum2n(nfld) /= dsum3n(nfld)) isum1 = isum1+1
            if (dsum2n(nfld) /= dsum4n(nfld)) isum1 = isum1+1
            if (rsum2n(nfld) /= rsum3n(nfld)) isum2 = isum2+1
         ...
         if (isum1 > 0) then
            print *, 'Multifield range/mask 2d double: FAIL',
         dsum2n,dsum3n,dsum4n else print *, 'Multifield range/mask 2d double:
         PASS' endif if (isum2 > 0) then print *, 'Multifield range/mask 2d
         real: FAIL', rsum2n,rsum3n,rsum4n else print *, 'Multifield range/mask
         2d real: PASS' endif if (isum3 > 0) then print *, 'Multifield
         range/mask 2d int: FAIL', isum2n,isum3n,isum4n else print *,
         'Multifield range/mask 2d int: PASS' endif if (isum4 > 0) then print *,
         'Multifield range/mask 2d int(8): FAIL', lsum2n,lsum3n,lsum4n else
         ...
         dsum4n = mpas_globalSumNfld(darray3n, dmask3n, MPI_COMM_WORLD,
         indxRange) rsum4n = mpas_globalSumNfld(rarray3n, rmask3n,
         MPI_COMM_WORLD, indxRange) isum4n = mpas_globalSumNfld(iarray3n,
         imask3n, MPI_COMM_WORLD, indxRange) lsum4n =
         mpas_globalSumNfld(larray3n, lmask3n, MPI_COMM_WORLD, indxRange)

         if (dsum1n(1) == dref .and. dsum1n(2) == dref .and. &
             dsum1n(3) == dref) then
            print *, 'Multi-field and reproducibility 3d double: PASS'
         else
            print *, 'Multi-field and reproducibility 3d double: FAIL', dref,
         dsum1n endif if (rsum1n(1) == rref .and. rsum1n(2) == rref .and. &
             rsum1n(3) == rref) then
            print *, 'Multi-field and reproducibility 3d real: PASS'
         else
            print *, 'Multi-field and reproducibility 3d real: FAIL', rref,
         rsum1n endif if (isum1n(1) == iref .and. isum1n(2) == iref .and. &
             isum1n(3) == iref) then
            print *, 'Multi-field and reproducibility 3d int: PASS'
         else
            print *, 'Multi-field and reproducibility 3d int: FAIL', iref,
         isum1n endif if (lsum1n(1) == lref .and. lsum1n(2) == lref .and. &
             lsum1n(3) == lref) then
            print *, 'Multi-field and reproducibility 3d int(8): PASS'
         else
            print *, 'Multi-field and reproducibility 3d int(8): FAIL', lref,
         lsum1n endif isum1 = 0 isum2 = 0 isum3 = 0 isum4 = 0 do nfld=1,nFields
            if (dsum2n(nfld) /= dsum3n(nfld)) isum1 = isum1+1
            if (dsum2n(nfld) /= dsum4n(nfld)) isum1 = isum1+1
            if (rsum2n(nfld) /= rsum3n(nfld)) isum2 = isum2+1
         ...
         if (isum1 > 0) then
            print *, 'Multifield range/mask 3d double: FAIL',
         dsum2n,dsum3n,dsum4n else print *, 'Multifield range/mask 3d double:
         PASS' endif if (isum2 > 0) then print *, 'Multifield range/mask 3d
         real: FAIL', rsum2n,rsum3n,rsum4n else print *, 'Multifield range/mask
         3d real: PASS' endif if (isum3 > 0) then print *, 'Multifield
         range/mask 3d int: FAIL', isum2n,isum3n,isum4n else print *,
         'Multifield range/mask 3d int: PASS' endif if (isum4 > 0) then print *,
         'Multifield range/mask 3d int(8): FAIL', lsum2n,lsum3n,lsum4n else
         ...
         dsum4n = mpas_globalSumNfld(darray4n, dmask4n, MPI_COMM_WORLD,
         indxRange) rsum4n = mpas_globalSumNfld(rarray4n, rmask4n,
         MPI_COMM_WORLD, indxRange) isum4n = mpas_globalSumNfld(iarray4n,
         imask4n, MPI_COMM_WORLD, indxRange) lsum4n =
         mpas_globalSumNfld(larray4n, lmask4n, MPI_COMM_WORLD, indxRange)

         if (dsum1n(1) == dref .and. dsum1n(2) == dref .and. &
             dsum1n(3) == dref) then
            print *, 'Multi-field and reproducibility 4d double: PASS'
         else
            print *, 'Multi-field and reproducibility 4d double: FAIL', dref,
         dsum1n endif if (rsum1n(1) == rref .and. rsum1n(2) == rref .and. &
             rsum1n(3) == rref) then
            print *, 'Multi-field and reproducibility 4d real: PASS'
         else
            print *, 'Multi-field and reproducibility 4d real: FAIL', rref,
         rsum1n endif if (isum1n(1) == iref .and. isum1n(2) == iref .and. &
             isum1n(3) == iref) then
            print *, 'Multi-field and reproducibility 4d int: PASS'
         else
            print *, 'Multi-field and reproducibility 4d int: FAIL', iref,
         isum1n endif if (lsum1n(1) == lref .and. lsum1n(2) == lref .and. &
             lsum1n(3) == lref) then
            print *, 'Multi-field and reproducibility 4d int(8): PASS'
         else
            print *, 'Multi-field and reproducibility 4d int(8): FAIL', lref,
         lsum1n endif isum1 = 0 isum2 = 0 isum3 = 0 isum4 = 0 do nfld=1,nFields
            if (dsum2n(nfld) /= dsum3n(nfld)) isum1 = isum1+1
            if (dsum2n(nfld) /= dsum4n(nfld)) isum1 = isum1+1
            if (rsum2n(nfld) /= rsum3n(nfld)) isum2 = isum2+1
         ...
         if (isum1 > 0) then
            print *, 'Multifield range/mask 4d double: FAIL',
         dsum2n,dsum3n,dsum4n else print *, 'Multifield range/mask 4d double:
         PASS' endif if (isum2 > 0) then print *, 'Multifield range/mask 4d
         real: FAIL', rsum2n,rsum3n,rsum4n else print *, 'Multifield range/mask
         4d real: PASS' endif if (isum3 > 0) then print *, 'Multifield
         range/mask 4d int: FAIL', isum2n,isum3n,isum4n else print *,
         'Multifield range/mask 4d int: PASS' endif if (isum4 > 0) then print *,
         'Multifield range/mask 4d int(8): FAIL', lsum2n,lsum3n,lsum4n else
         ...
         dsum4n = mpas_globalSumNfld(darray5n, dmask5n, MPI_COMM_WORLD,
         indxRange) rsum4n = mpas_globalSumNfld(rarray5n, rmask5n,
         MPI_COMM_WORLD, indxRange) isum4n = mpas_globalSumNfld(iarray5n,
         imask5n, MPI_COMM_WORLD, indxRange) lsum4n =
         mpas_globalSumNfld(larray5n, lmask5n, MPI_COMM_WORLD, indxRange)

         if (dsum1n(1) == dref .and. dsum1n(2) == dref .and. &
             dsum1n(3) == dref) then
            print *, 'Multi-field and reproducibility 5d double: PASS'
         else
            print *, 'Multi-field and reproducibility 5d double: FAIL', dref,
         dsum1n endif if (rsum1n(1) == rref .and. rsum1n(2) == rref .and. &
             rsum1n(3) == rref) then
            print *, 'Multi-field and reproducibility 5d real: PASS'
         else
            print *, 'Multi-field and reproducibility 5d real: FAIL', rref,
         rsum1n endif if (isum1n(1) == iref .and. isum1n(2) == iref .and. &
             isum1n(3) == iref) then
            print *, 'Multi-field and reproducibility 5d int: PASS'
         else
            print *, 'Multi-field and reproducibility 5d int: FAIL', iref,
         isum1n endif if (lsum1n(1) == lref .and. lsum1n(2) == lref .and. &
             lsum1n(3) == lref) then
            print *, 'Multi-field and reproducibility 5d int(8): PASS'
         else
            print *, 'Multi-field and reproducibility 5d int(8): FAIL', lref,
         lsum1n endif isum1 = 0 isum2 = 0 isum3 = 0 isum4 = 0 do nfld=1,nFields
            if (dsum2n(nfld) /= dsum3n(nfld)) isum1 = isum1+1
            if (dsum2n(nfld) /= dsum4n(nfld)) isum1 = isum1+1
            if (rsum2n(nfld) /= rsum3n(nfld)) isum2 = isum2+1
         ...
         if (isum1 > 0) then
            print *, 'Multifield range/mask 5d double: FAIL',
         dsum2n,dsum3n,dsum4n else print *, 'Multifield range/mask 5d double:
         PASS' endif if (isum2 > 0) then print *, 'Multifield range/mask 5d
         real: FAIL', rsum2n,rsum3n,rsum4n else print *, 'Multifield range/mask
         5d real: PASS' endif if (isum3 > 0) then print *, 'Multifield
         range/mask 5d int: FAIL', isum2n,isum3n,isum4n else print *,
         'Multifield range/mask 5d int: PASS' endif if (isum4 > 0) then print *,
         'Multifield range/mask 5d int(8): FAIL', lsum2n,lsum3n,lsum4n else
      ...
      */
      /*
            ...
            if (MyResI4 != expI4)
               ABORT_ERROR("Global sum A1DI4: FAIL (exp,act={}, {})", expI4,
         MyResI4);

            globalSum(HostArr2DI4, Comm, &MyResI4);
            expI4 = Sum2DI4 * MySize;
            if (MyResI4 != expI4)
               ABORT_ERROR("Global sum A2DI4: FAIL (exp,act={}, {})", expI4,
         MyResI4);
            ...
            if (MyResI8 != expI8)
               ABORT_ERROR("Global sum A1DI8: FAIL (exp,act={}, {})",expI8,
         MyResI8);

            globalSum(HostArr2DI8, Comm, &MyResI8);
            expI8 = Sum2DI8 * MySize;
            if (MyResI8 != expI8)
               ABORT_ERROR("Global sum A2DI8: FAIL (exp,act={}, {})", expI8,
         MyResI8);
            ...
            if (MyResR4 != expR4)
               ABORT_ERROR("Global sum A1DR4: FAIL (exp,act={}, {})", expR4,
         MyResR4);

            globalSum(HostArr2DR4, Comm, &MyResR4);
            expR4 = Sum2DR4 * MySize;
            if (MyResR4 != expR4)
               ABORT_ERROR("Global sum A2DR4: FAIL (exp,act={}, {})", expR4,
         MyResR4);
            ...
            for (i = 0; i < NumCells; i++) {
               ...
               e  = t1 - ai;
               t2 = ((real(LocalSum1D) - e) + (ai - (t1 - e))) +
         imag(LocalSum1D); LocalSum1D = complex<double>(t1 + t2, t2 - ((t1 + t2)
         - t1)); for (j = 0; j < NumVertLyrs; j++) { HostArr2DR8(i, j) = D; D
         += 1.0000000000001; ai = HostArr2DR8(i, j); t1 = ai + real(LocalSum2D);
                  e  = t1 - ai;
                  t2 = ((real(LocalSum2D) - e) + (ai - (t1 - e))) +
         imag(LocalSum2D); LocalSum2D = complex<double>(t1 + t2, t2 - ((t1 + t2)
         - t1));
               }
            }
            ...
            for (i = 0; i < MySize; i++) {
               ...
               e  = t1 - Sum1DR8;
               t2 = ((real(SerialSum) - e) + (Sum1DR8 - (t1 - e))) +
         imag(SerialSum); SerialSum = complex<double>(t1 + t2, t2 - ((t1 + t2) -
         t1));
            }
            expR8 = real(SerialSum);
            if (MyResR8 != expR8)
               ABORT_ERROR("Global sum A1DR8: FAIL (exp,act={}, {})", expR8,
         MyResR8);

            globalSum(HostArr2DR8, Comm, &MyResR8);
            expR8 = Sum2DR8 * MySize;
            if (MyResR8 != expR8)
               ABORT_ERROR("Global sum A2DR8: FAIL (exp,act={}, {})", expR8,
         MyResR8);
            ...
            if (MyResI8 != MySize - 1)
               ABORT_ERROR("Global max I4:    FAIL (exp,act={}, {})", MySize -
         1, MyResI8);

            R8 MyR8Tmp = MyTask + MyR8;
            globalMin(&MyR8Tmp, &MyResR8, Comm);
            if (MyResR8 != MyR8)
               ABORT_ERROR("Global min R8:    FAIL (exp,act={}, {})", MyR8,
         MyResR8);
            ...
            for (j = 0; j < NumCells; j++) {
               for (i = 0; i < MySize; i++) {
                  k = i * NumCells + j;
                  if (MyTask != i) {
                     HostA1DI4Work(k) = (i + 1) * k; // processor-specific work
         array } else { HostA1DI4Work(k) = k; // default
                  }
               }
            }
         ...
         }); parallelFor( {NumCells, NumVertLyrs}, KOKKOS_LAMBDA(int i, int j) {
         DevArr2DI4(i, j) = i * 10 + j; }); Kokkos::fence();

            globalSum(DevArr1DI4, Comm, &MyResI4);
            expI4 = Sum1DI4 * MySize;
            if (MyResI4 != expI4)
               ABORT_ERROR("Global sum device A1DI4: FAIL (exp,act={}, {})",
         expI4, MyResI4);

            globalSum(DevArr2DI4, Comm, &MyResI4);
            expI4 = Sum2DI4 * MySize;
            if (MyResI4 != expI4)
               ABORT_ERROR("Global sum device A2DI4: FAIL (exp,act={}, {})",
         expI4, MyResI4);
            ...
            expR4 = Sum1DR4 * MySize;
            if (MyResR4 != expR4)
               ABORT_ERROR("Global sum device A1DR4: FAIL (exp,act={}, {}",
         expR4, MyResR4);
      */
